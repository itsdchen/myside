...
#
# Note: copied verbatim from public-facing hyperliquid sdk.
# Version 0.3.0, 20240328

from hl_types import Literal, Optional, TypedDict, Union, Cloid, NotRequired
...
def sign_l1_action(wallet, action, active_pool, nonce, is_mainnet):
    # Let's check on this, yeah?
    # wallet: given
    # order_action, we know
    # vault_address: should be None
    # timestamp = nonce
    # base_url = blah
    ...
    # b'\x85\xc0\xab\xaaW`~\xac&\x96^\x03\x85mSH\xa4\xc9\x92\xe3#\x07\xc9V/\xe4.\xfc\xc1\xdfk\x81'
    phantom_agent = construct_phantom_agent(hash, is_mainnet)
    # {'source': 'b', 'connectionId': b'\x85\xc0\xab\xaaW`~\xac&\x96^\x03\x85mSH\xa4\xc9\x92\xe3#\x07\xc9V/\xe4.\xfc\xc1\xdfk\x81'}
    data = {
        "domain": {
            "chainId": 1337,
            "name": "Exchange",
    ...
    # {'domain': {'chainId': 1337, 'name': 'Exchange', 'verifyingContract': '0x0000000000000000000000000000000000000000', 'version': '1'}, 'types': {'Agent': [{'name': 'source', 'type': 'string'}, {'name': 'connectionId', 'type': 'bytes32'}], 'EIP712Domain': [{'name': 'name', 'type': 'string'}, {'name': 'version', 'type': 'string'}, {'name': 'chainId', 'type': 'uint256'}, {'name': 'verifyingContract', 'type': 'address'}]}, 'primaryType': 'Agent', 'message': {'source': 'b', 'connectionId': b'\x85\xc0\xab\xaaW`~\xac&\x96^\x03\x85mSH\xa4\xc9\x92\xe3#\x07\xc9V/\xe4.\xfc\xc1\xdfk\x81'}}
