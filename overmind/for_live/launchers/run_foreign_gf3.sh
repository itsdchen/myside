...
case "$STRATEGY" in
    day)
        DESCRIPTION="Running $EQUITY day for $TODAY"
        STRAT_ID="${EQUITY}day"
        CMD=("$PK_EXEC" --conf "$EQUITY/day/pk_day.json" --date "$TODAY"  --end-date "$TOMORROW" --mainnet \
             --out-dir "$EQUITY/day" --full-output --live --init-sleep-secs 15 -i "$STRAT_ID")
        STDOUT_LOG="$LOG_DIR/pk_stdout_${STRAT_ID}_${TODAY}.txt"
        ;;
    day_narrow)
        DESCRIPTION="Running $EQUITY day_narrow for $TODAY"
        STRAT_ID="${EQUITY}day_narrow"
        CMD=("$PK_EXEC" --conf "$EQUITY/day_narrow/pk_day_narrow.json" --date "$TODAY"  --end-date "$TOMORROW" --mainnet \
             --out-dir "$EQUITY/day_narrow" --full-output --live --init-sleep-secs 15 -i "$STRAT_ID")
        STDOUT_LOG="$LOG_DIR/pk_stdout_${STRAT_ID}_${TODAY}.txt"
        ;;

    btc)
        DESCRIPTION="Running $EQUITY btc for $TODAY"
        STRAT_ID="${EQUITY}_btc"
        CMD=("$PK_EXEC" --conf "$EQUITY/btc/pk_btc.json" --date "$TODAY"  --end-date "$TOMORROW" --mainnet \
             --out-dir "$EQUITY/btc" --full-output --live --init-sleep-secs 15 -i "$STRAT_ID" --beacon-always-deliver)
        STDOUT_LOG="$LOG_DIR/pk_stdout_${STRAT_ID}_${TODAY}.txt"
        ;;

    btc_narrow)
        DESCRIPTION="Running $EQUITY btc_narrow for $TODAY"
        STRAT_ID="${EQUITY}_btc_narrow"
        CMD=("$PK_EXEC" --conf "$EQUITY/btc_narrow/pk_btc_narrow.json" --date "$TODAY"  --end-date "$TOMORROW" --mainnet \
             --out-dir "$EQUITY/btc_narrow" --full-output --live --init-sleep-secs 15 -i "$STRAT_ID" --beacon-always-deliver)
        STDOUT_LOG="$LOG_DIR/pk_stdout_${STRAT_ID}_${TODAY}.txt"
        ;;
    *)
