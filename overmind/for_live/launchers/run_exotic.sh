...
case "$STRATEGY" in
    allday)
        ...
        STRAT_ID="${EQUITY}allday"
        CMD=("$PK_EXEC" --conf "$EQUITY/allday/pk_allday.json" --date "$TODAY"  --end-date "$TOMORROW" --mainnet \
             --out-dir "$EQUITY/allday" --full-output --live --init-sleep-secs 15 -i "$STRAT_ID")
